"""Gated DeltaNet 2 sequence mixer (Triton) -- thin wrapper over fla.layers.GatedDeltaNet2.

This is the fast `gdn2_triton` mixer: it runs fla's Triton chunk/fused_recurrent kernels
and needs a CUDA GPU (Ampere+ in practice; bf16). For a pure-PyTorch, Triton-free port that
runs on CPU or a Turing/T4 GPU, use the default `gdn2` mixer in `newattn.mixers.gdn2`.
